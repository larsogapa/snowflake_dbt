...
from AIRBNB.raw.raw_orders as orders
join (
      select 
        first_name || ' ' || last_name as name, 
        * 
      from {{ source('jaffe_shop', 'raw_customers') }}
) customers
...
join (
    ...
    from (
      select 
        ...
        *
      from {{ source('jaffe_shop', 'raw_orders') }}
    ) a
    join ( 
      select 
        first_name || ' ' || last_name as name, 
    ...
    on a.user_id = b.id
    left outer join AIRBNB.raw.raw_payments c
    on a.id = c.order_id
    where a.status NOT IN ('pending') and a.status != 'fail'
    group by b.id, b.name, b.last_name, b.first_name
) customer_order_history
on orders.user_id = customer_order_history.customer_id
left outer join {{ source('jaffe_shop', 'raw_payments') }} payments
